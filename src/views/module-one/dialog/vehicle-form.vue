<template>
  <div>
    <cm-dialog :title="dialogTitle" v-model="dialogFormVisible.status">
      <el-form class="m-form-inline" :model="model" :inline="true" :label-position="'right'" label-width="100px">
        ...
        <el-form-item label="驾驶资格要求">
          ...
          <el-select v-model="drivingQualification" multiple placeholder="请选择">
            <el-option v-for="item in drivingQualificationOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="停用日期">
          <cm-date-picker v-model="model.endTime" v-show="!long_use"></cm-date-picker>
        </el-form-item>
...
export default {
  ...
  created () {
    if (this.formModel.id > 0) {
      ...
      this.model = model
      this.long_use = this.model.endTime === '9999-01-01'
